...
export class CreateAccountComponent {
  ...
  getCustomerDetails() {
    this.customerService.getCoustomer(this.deviceValue?.Dealer?.Id, this.deviceValue?.Customer?.Id).subscribe((res: any) => {
  ...
  submit(formvalue: any) {
    ...
    let payload = {
      "Id": Number(this.deviceValue?.Customer?.Id),
      "DealerId": Number(this.deviceValue?.Dealer?.Id),
      "CustomerName": formvalue.customerName,
      "User": {
        "Id": Number(this.customerData?.User?.Id),
        "CustomerId": Number(this.deviceValue?.Customer?.Id),
        "LoginId": formvalue?.userName,
        "Password": formvalue?.password,
        "MobileNo": null,
        "Type": 1,
        "Reseller": {
          "Id": Number(this.deviceValue?.Dealer?.Id)
        }
      },
    ...
    this.customerService.updateCustomer(Number(this.deviceValue?.Customer?.Id), payload).subscribe((res: any) => {
